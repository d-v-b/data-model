"""
Pydantic-zarr integrated models for Sentinel-2B EOPF Zarr data structure.

Uses the new pyz.GroupSpec with TypedDict members to enforce strict structure validation.

Note: Sentinel-2B data shares the same structure as Sentinel-2A and Sentinel-2C.
This module primarily re-exports the base Sentinel2Root model as Sentinel2BRoot.
"""
...
class Sentinel2RootAttrs(BaseModel):
    ...
    other_metadata4: dict[str, object] | None = None  # Sentinel-2B specific metadata
...
class Sentinel2QualityMembers(TypedDict, closed=True, total=False):  # type: ignore[call-arg]
    ...
    Closed TypedDict - for Sentinel-2B contains l1c_quicklook and mask keys.
    All fields are optional to accommodate different product levels.
    """
...
class Sentinel2QualityGroup(GroupSpec[DatasetAttrs, Sentinel2QualityMembers]):  # type: ignore[type-var]
    """Quality group containing L1C quicklook and classification data for Sentinel-2B."""

    def quicklook(self) -> Sentinel2QuicklookDataset | None:
        """Get L1C quicklook subgroup."""
        return self.members.get("l1c_quicklook")
...
class Sentinel2BRoot(GroupSpec[Sentinel2RootAttrs, Sentinel2RootMembers]):  # type: ignore[type-var]
...
# Sentinel-2B Root alias
Sentinel2BRoot = Sentinel2BRoot
Sentinel2BRootAttrs = Sentinel2RootAttrs
