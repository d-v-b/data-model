...
import re
from eopf_geozarr.data_api.geozarr.v2 import CoordArray, CoordArrayAttrs, MyGroupSpec, check_valid_coordinates
...
@pytest.mark.parametrize("name", ["air_temperature", "sea_surface_temperature", "precipitation_flux"])
...
def test_coord_array_attrs_dimensions_length() -> None:
    ...
    """
    msg = ('1 validation error for CoordArrayAttrs\n_ARRAY_DIMENSIONS\n '
...
def test_coord_array_dimensionality() -> None:
    ...
    """
    msg = ('1 validation error for CoordArray\nshape\n  '
           'Tuple should have at most 1 item after validation, not 2')
    with pytest.raises(ValueError, match=re.escape(msg)):
        CoordArray(
            shape=(10, 11),
...
class TestCheckValidCoordinates:
    @pytest.mark.parametrize("example", [
        ...
        "/time": CoordArray(shape=(10,), dtype='|u1', chunks=(10,), attributes=CoordArrayAttrs(
    ...
    @pytest.mark.parametrize("example", [
        {
            ...
            "/time": ArraySpec(shape=(10,), dtype='|u1', chunks=(10,), attributes=CoordArrayAttrs(
                _ARRAY_DIMENSIONS=["time"],
                standard_name="time",
                units="s",
                axis="T"
            )),
            "/lat": ArraySpec(shape=(11,), dtype='|u1', chunks=(11,), attributes=CoordArrayAttrs(
