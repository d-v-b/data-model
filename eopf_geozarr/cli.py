...
def _generate_html_output(dt: xr.DataTree, output_path: str, input_path: str, verbose: bool = False) -> None:
    ...
    """
    try:
        # Get the HTML representation of the DataTree
        # This uses xarray's built-in HTML representation
        html_repr = dt._repr_html_()
...
<head>
    ...
    <style>
        ...
        /* Enhance xarray's default styling */
        .xr-section-item {{
            margin-bottom: 10px;
        }}
        .xr-section-summary {{
...
<body>
    <div class="container">
        <div class="header">
            <h1>DataTree Visualization</h1>
            <p><strong>Dataset:</strong> {input_path}</p>
            <p><strong>Total groups:</strong> {len(dt.children)}</p>
